...
class Map(object):
    ...
    def __getattr__(self, key):
        ...
        try:
            ...
            print(
                "[Map] Key '{}' is not available. Available keys are : {}".format(
                    key, str(list(self._map.keys()))
                )
            )
...
class MapAccess(object):
    ...
    def __getattr__(self, key):
        ...
        if key not in self._input:
            raise AttributeError(
                "[Map Access] '{}' is not declared in inputs, so read permission denied. [inputs: {}]".format(
                    key, self._input
                )
            )
    ...
    def __setattr__(self, key, value):
        if "_MapAccess__initialized" not in self.__dict__:
            return object.__setattr__(self, key, value)
        elif key in ["_mapObj", "_input", "_output"] and (
            "_MapAccess__initialized" in self.__dict__
        ):
            raise AttributeError("[MapAcess] '{}' is read-only attribute".format(key))

        if key not in self._output:
            raise AttributeError(
                "[Map Access] '{}' is not declared in outputs, so write permission denied. [outputs: {}]".format(
                    key, self._output
                )
            )
