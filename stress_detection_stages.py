...
logging.basicConfig(
    level=logging.ERROR, format="[%(levelname)s] [{}] %(message)s".format(time.time())
)
...
class LoadModels(Stage):
    ...
    def execute(self, io, static_io):
        ...
        emotion_model_json_file, emotion_model_weights_file = static_io.model_files[
            "emotion_detection_model"
        ]
        strees_classifier_model_sav_file = static_io.model_files[
            "stress_classifier_model"
        ]
        ...
        try:
            ...
            stress_classifier_model = pickle.load(
                open(strees_classifier_model_sav_file, "rb")
            )
...
class GrabFrame(Stage):
    def __init__(self):
        super(GrabFrame, self).__init__(
            input_keys=["frame"], output_keys=["gray_frame"]
        )
...
class FaceDetection(Stage):
    def __init__(self):
        super(FaceDetection, self).__init__(
            input_keys=["gray_frame", "models"], output_keys=["faces"]
        )

    def execute(self, io, static_io):
        try:
            faces = static_io.models["face_detection_model"].detectMultiScale(
                io.gray_frame
            )
        except Exception as e:
            logging.error(e)
...
class EmotionDetection(Stage):
    def __init__(self):
        super(EmotionDetection, self).__init__(
            input_keys=["faces", "gray_frame", "models"], output_keys=["predictions"]
        )
    ...
    def execute(self, io, static_io):
        ...
        for (x, y, w, h) in io.faces:
            ...
            try:
                predictions.append(
                    static_io.models["emotion_detection_model"]
                    .predict(face[np.newaxis, :, :, np.newaxis])
                    .tolist()[0]
                )
            except Exception as e:
                logging.error(e)
...
class StressClassifier(Stage):
    def __init__(self):
        super(StressClassifier, self).__init__(
            input_keys=["predictions", "models"], output_keys=["stress_scores"]
        )

    def execute(self, io, static_io):
        classifier_predictions = static_io.models[
            "stress_classifier_model"
        ].predict_proba(io.predictions)
        io.stress_scores = self.calculate_score(
            classifier_predictions, [0.3, 0.01, 0.90]
        ).round(2)
        return status.SUCCESS
    ...
    def calculate_score(self, predict_probabilites, target_weights):
        ...
        for prediction in predict_probabilites:
            ...
            for target_index in range(len(prediction)):
                score += (
                    target_weights[target_index] * prediction[target_index]
                ) / normalising_value
...
class GenerateDisplayFrame(Stage):
    def __init__(self):
        super(GenerateDisplayFrame, self).__init__(
            input_keys=["frame", "faces", "display_frame", "stress_scores"],
            output_keys=["display_frame", "display_frame_bytes"],
        )
    ...
    def execute(self, io, static_io):
        ...
        for (x, y, w, h) in io.faces:
            ...
            radius = int(w / 2)
            cv2.circle(
                io.display_frame, (center_x, center_y), radius, (0, 255, 0), thickness
            )
